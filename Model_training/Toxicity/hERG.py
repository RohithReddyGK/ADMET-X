# hERG.py (auto handles classification vs regression + units + auto XGBoost type)
...
dataset_units = {
    ...
    "Half_Life_Obach": "hours",
    "Clearance_Hepatocyte_AZ": "mL/min/kg",
    "Clearance_Microsome_AZ": "mL/min/mg",
    "hERG": "IC50 (µM)",
    "AMES": "-",
    "DILI": "-",
    "SkinReaction": "-",
...
# ---------- Druglikeness rules ----------
def lipinski_rule(mol): return (Descriptors.MolWt(mol)<=500 and Crippen.MolLogP(mol)<=5 and Lipinski.NumHDonors(mol)<=5 and Lipinski.NumHAcceptors(mol)<=10)
def ghose_rule(mol): mw, logp, atoms = Descriptors.MolWt(mol), Crippen.MolLogP(mol), mol.GetNumAtoms(); return (160<=mw<=480 and -0.4<=logp<=5.6 and 20<=atoms<=70)
def veber_rule(mol): return (rdMolDescriptors.CalcTPSA(mol)<=140 and rdMolDescriptors.CalcNumRotatableBonds(mol)<=10)
def egan_rule(mol): return (rdMolDescriptors.CalcTPSA(mol)<=131 and Crippen.MolLogP(mol)<=5.88)
def muegge_rule(mol): mw, logp = Descriptors.MolWt(mol), Crippen.MolLogP(mol); return (200<=mw<=600 and -2<=logp<=5 and Lipinski.NumHAcceptors(mol)<=10 and Lipinski.NumHDonors(mol)<=5 and rdMolDescriptors.CalcTPSA(mol)<=150)

# ---------- Dataset auto-detection ----------
def auto_detect_dataset_name(file_path: str) -> str:
    fname = os.path.basename(file_path).lower()
    for key in dataset_units.keys():
        if key.lower() in fname: return key
    parts = [p.lower() for p in file_path.split(os.sep)]
    for key in dataset_units.keys():
        if key.lower() in parts: return key
    return "Unknown"

# ---------- Train model ----------
def train_model(train_csv, valid_csv, smiles_col="Drug", label_col="Y",
                save_model_path=None, use_descriptors=True, use_fingerprints=True, model_type="rf"):

    train_df, valid_df = pd.read_csv(train_csv), pd.read_csv(valid_csv)
    ...
    dataset_name = auto_detect_dataset_name(train_csv)
    is_classification = dataset_name not in regression_datasets

    # ---------- Model selection ----------
    if is_classification:
        print(f"Task detected: Classification ({dataset_name})")
        if model_type=="rf":
            model = RandomForestClassifier(n_estimators=300, random_state=42, class_weight="balanced", n_jobs=-1)
        else:  # XGBoost automatically picks classifier
        ...
        print(f"Task detected: Regression ({dataset_name})")
        if model_type=="rf":
            model = RandomForestRegressor(n_estimators=300, random_state=42, n_jobs=-1)
        else:  # XGBoost automatically picks regressor
            model = XGBRegressor(
                n_estimators=500, learning_rate=0.05, max_depth=6,
                subsample=0.8, colsample_bytree=0.8,
                random_state=42, n_jobs=-1
    ...
    if X_valid.shape[0] > 0:
        ...
        if is_classification:
            try: auc = roc_auc_score(y_valid, model.predict_proba(X_valid)[:,1])
            except: auc = None
            print("\nValidation results:")
...
def evaluate_and_save_predictions(model_meta, test_csv, prediction_csv=None, smiles_col="Drug", label_col="Y"):
    ...
    dataset_name = model_meta.get("dataset_name", auto_detect_dataset_name(test_csv))
    ...
    if desc_test_df is not None and not desc_test_df.empty:
        for c in desc_test_df.columns: result.loc[desc_test_df.index, c] = desc_test_df[c].values
    ...
    if X_test.shape[0] > 0:
        ...
        if is_classification:
            ...
            except: probs = np.full((X_test.shape[0], 2), np.nan)
            classes = list(getattr(model,"classes_",[0,1]))
            for cls in classes: result[f"Prob_Class_{cls}"] = np.nan
            for i, idx in enumerate(idx_test):
                result.at[idx, "Predicted_Class"] = y_pred[i]
                if probs.ndim==2:
                    for j, cls in enumerate(classes): result.at[idx,f"Prob_Class_{cls}"]=probs[i,j]
        else:
            result["Predicted_Value"] = np.nan
            for i, idx in enumerate(idx_test): result.at[idx,"Predicted_Value"]=y_pred[i]
    else:
        if is_classification:
            result["Predicted_Class"] = np.nan
    ...
    for rule_name, rule_func in zip(["Lipinski","Ghose","Veber","Egan","Muegge"],
        ...
        result[rule_name] = ["Invalid" if Chem.MolFromSmiles(smi) is None else ("Yes" if rule_func(Chem.MolFromSmiles(smi)) else "No")
                             for smi in result[smiles_col].astype(str)]

    # Units
    result["Units"] = dataset_units.get(dataset_name,"-")

    # Save CSV
    if prediction_csv:
        os.makedirs(os.path.dirname(prediction_csv), exist_ok=True)
        result.to_csv(prediction_csv, index=False)
        print(f"\n[{dataset_name}] Predictions saved to: {prediction_csv}")
        print(result.head(5))
...
if __name__ == "__main__":
    train_csv = r"D:\VS Code Editor\Major Project\ADMET\admet_data\Toxicity\hERG\train.csv"
    valid_csv = r"D:\VS Code Editor\Major Project\ADMET\admet_data\Toxicity\hERG\valid.csv"
    test_csv  = r"D:\VS Code Editor\Major Project\ADMET\admet_data\Toxicity\hERG\test.csv"

    save_model_path = r"D:\VS Code Editor\Major Project\ADMET\Model_training\Toxicity\hERG.joblib"
    prediction_csv = r"D:\VS Code Editor\Major Project\ADMET\Model_predictions\Toxicity\hERG.csv"
